...
const PAIRS = [
  { id: 'session', term: 'Session', ticket: 'Started a new chat for this subtask, instead of dragging in the last one.' },
  { id: 'rag', term: 'RAG', ticket: 'It searched the docs first, then answered using what it found.' },
  { id: 'hallucination', term: 'Hallucination', ticket: 'It answered fast and sounded sure. It was completely wrong.' },
  { id: 'contextwindow', term: 'Context window', ticket: 'Ask about the first message now, and it says it doesn’t see it anymore.' },
  { id: 'agent', term: 'Coding agent', ticket: 'Read the file, edited it, ran the tests, all on its own.' },
  { id: 'skill', term: 'Skill', ticket: 'Pulled up the team’s saved checklist instead of figuring out the steps from scratch.' },
]

const TERM_ORDER = ['hallucination', 'agent', 'skill', 'session', 'contextwindow', 'rag']
const TERMS = TERM_ORDER.map((id) => PAIRS.find((p) => p.id === id))

function Hour4MatchGameSlide() {
      ...
      <div className="mt-6 flex min-h-[3rem] max-w-2xl flex-col items-center gap-3 text-center text-sm text-white/60">
        {allMatched ? (
          <motion.p initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}>
            All six matched. That&rsquo;s the whole vocabulary, frontend to AI, everything
            you&rsquo;ll see in a ticket.
          </motion.p>
        ) : (
...
export default Hour4MatchGameSlide
